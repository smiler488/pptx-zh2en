...
```bash
ls /root/uploads/*.pptx 2>/dev/null
ls /workspace/*.pptx 2>/dev/null
```

用 markitdown 预览内容，**判断 PPT 所属领域**：

```bash
python3 -m markitdown /path/to/input.pptx
```
...
```bash
# 中→英（默认）
python3 /root/.codebuddy/skills/pptx-zh2en/scripts/translate_pptx_inline.py \
  --mode extract --direction zh2en \
  --input /path/to/chinese.pptx \
  --translations-file /workspace/translations.json

# 英→中
python3 /root/.codebuddy/skills/pptx-zh2en/scripts/translate_pptx_inline.py \
  --mode extract --direction en2zh \
  --input /path/to/english.pptx \
  --translations-file /workspace/translations.json
```
...
python3 /root/.codebuddy/skills/pptx-zh2en/scripts/translate_pptx_inline.py \
  ...
  --input /path/to/chinese.pptx \
  --translations-file /workspace/translations.json \
  --output /workspace/<原文件名>_EN.pptx
...
python3 /root/.codebuddy/skills/pptx-zh2en/scripts/translate_pptx_inline.py \
  ...
  --input /path/to/english.pptx \
  --translations-file /workspace/translations.json \
  --output /workspace/<原文件名>_ZH.pptx
```
...
python3 /root/.codebuddy/skills/pptx-zh2en/scripts/translate_pptx.py \
  --input /path/to/chinese.pptx \
  --output /workspace/<原文件名>_EN.pptx \
  --model gpt-4o-mini
```
...
最终文件保存至 `/workspace/<原文件名>_EN.pptx`
